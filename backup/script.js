...
function enableUniverseScroll() {
  ...
  function onWheel(e) {
    if (closing) return;
    scrollTotal += Math.abs(e.deltaY);
    if (scrollTotal >= scrollThreshold) {
      closing = true;
      closeUniverse();
      uniCont.removeEventListener('wheel', onWheel);
    }
  }
  ...
  function onTouchMove(e) {
    ...
    if (delta >= scrollThreshold) {
      closing = true;
      closeUniverse();
      uniCont.removeEventListener('touchstart', onTouchStart);
      uniCont.removeEventListener('touchmove', onTouchMove);
    }
  }

  uniCont.addEventListener('wheel', onWheel);
  uniCont.addEventListener('touchstart', onTouchStart);
  uniCont.addEventListener('touchmove', onTouchMove);
}
...
function initUniverse() {
  ...
  function anim() {
    ...
    comets.forEach((co, i) => {
      ...
      const headGrad = ctx.createRadialGradient(co.x, co.y, 0, co.x, co.y, co.size * 3);
      ...
      ctx.lineTo(co.x - Math.cos(co.angle) * co.length, co.y - Math.sin(co.angle) * co.length);
      ctx.stroke();
    });
